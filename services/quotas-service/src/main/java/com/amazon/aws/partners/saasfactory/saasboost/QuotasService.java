...
public class QuotasService implements RequestHandler<Map<String, Object>, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent checkQuotas(Map<String, Object> event, Context context) {
        ...
        QuotasServiceDAL.QuotaCheck quotaCheck = null;
        if (Utils.isCnRegion(AWS_REGION)) {
            // no QuotasService in China regions
            LOGGER.info("QuotasService is not available in region " + AWS_REGION + ", ignore checkQuotas");
            quotaCheck = new QuotasServiceDAL.QuotaCheck();
            quotaCheck.setPassed(true);
            quotaCheck.setMessage("no QuotasService in China regions, set all to pass");
        } else {
